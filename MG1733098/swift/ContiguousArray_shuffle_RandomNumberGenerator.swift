 ...
 * @className: ContiguousArray_shuffle_RandomNumberGenerator
 * @apiSignature: ContiguousArray$mutating func shuffle<T>(using generator: inout T) where T : RandomNumberGenerator
 * @description: Test swift api ContiguousArray$mutating func shuffle<T>(using generator: inout T) where T : RandomNumberGenerator
 * @Map: Collections$public static void shuffle (List<?> list, Random rnd)
 */
class ContiguousArray_shuffle_RandomNumberGenerator {
    /**
     * input: 2
     * class0 class0=[1,2,3]
...
ContiguousArray_shuffle_RandomNumberGenerator.shuffle0()
